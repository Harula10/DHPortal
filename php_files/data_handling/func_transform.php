<?php	
	...
	function split_it($func){
	    ...
	    for($i = 0; $i < strlen($func); $i++){
	        ...
	        if($func[$i]==',' & $count==0){
	            $parameters[$j] = substr($func, 0, $i);
	            $func = substr($func, $i+1, strlen($func));
	            $parameters[++$j] = $func;
	            $i=0;
	            $flag=false;
	            //$j++;
	...
	function splitExpression($function){
    	...
    	for($i = 0; $i < count($parameters); $i++){
			...
			if(preg_match("/.*\(.*\)/",$parameters[$i])){
				...
				//echo "3 ".$parameters[$i]."\n";
				if($parameters[$i]!="")
				    $parameters[$i] = splitExpression(trim($parameters[$i])).$meta;
				//echo "4 ".$parameters[$i]."\n\n";
			}
	    }
		// echo "2 ".$f."\n";
	...
	function replaceFunctionText($functionName, array $parameters){
		...
		if($output=='null'||$output=='current_date'){
	    	return $output;
		}
		return "(".$output.")";
	}
